...
##### 회고
- 왕실의 나이트, 게임개발 문제를 풀었다. 지난 번 스터디에서 map을 사용해서 푸는 방식을 보고 map을 사용해보려고 했는데 오히려 더 복잡해졌다.. 그래도 다음에 적절한 문제에 사용할 수 있을 것 같다.
...
##### 회고
- 상하좌우, 시각 2개의 문제를 풀었다. 상하좌우 문제를 풀 때 문자열 입력을 받기 위해 getline을 사용했는데 cin 이후에 getline을 받으니까 \n(줄바꿈)때문에 제대로 입력이 받아지지 않았다. 그래서 \n을 무시할 수 있도록 cin.ignore()함수를 사용했다.
...
##### 회고
- 1이 될 때까지 문제를 풀었다. 그리디 문제라고해서 단순하게만 풀지말고 어떻게하면 문제를 간략하게 풀 수 있을지 고민하고 계속해서 코드를 업그레이드 시킬 수 있도록 공부해야할 것 같다.
같은 문제를 풀더라도 메모리, 타임 등 자원을 적게 사용할 수 있도록 노력해야겠다.
...
##### 회고
- 숫자 카드 게임 문제를 풀고 반복문을 줄일 수 있는 방법에 대해서 고민했다.
...
##### 회고
- '이것이 취업을 위한 코딩테스트다' 교재를 활용하여 코딩테스트를 준비해 볼 예정이다.
오랜만에 c++를 이용하여 코딩했더니 생각보다 너무 재밌었다. 매주 3~5개 정도의 문제 풀이를 해 볼 예정이다.
